...
// // Load embedding model (use Python-based service in prod)
// const embedModel = new SentenceTransformer("BAAI/bge-m3");
...
//     const embedding = await embedModel.encode(combined);

//     const context = await getRelevantChunks(embedding);
...
// // Load the USE model once at server start
// (async () => {
...
//   try {
//     if (!embedModel) {
//       return res.status(503).json({ error: "Embedding model is not loaded yet, please try again shortly." });
//     }

//     const combined = `${mood} ${journal} ${userInput}`;
//     const embeddingsTensor = await embedModel.embed([combined]);
//     const embedding = embeddingsTensor.arraySync()[0]; // get embedding as plain array
...
// // app.get("/" (req,res) => {
// //     res.send("Welcome to the Therapy Chatbot API. Use POST /chat to interact with the chatbot.");
// // })
